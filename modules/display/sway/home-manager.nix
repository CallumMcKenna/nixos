...
in
{
  ...
  config = lib.mkIf config.defaultPkgs.enable {
    # Wallpaper app
    services.swww.enable = lib.mkIf config.defaultPkgs.sway.enable true;

    wayland.windowManager.sway = lib.mkIf config.defaultPkgs.sway.enable {
      enable = true;
      config = {
        # Launch waybar on startup
        startup = [
          { command = "waybar"; }
          { command = "swww img ~/config/modules/display/wallpaper.jpg"; }
        ];
